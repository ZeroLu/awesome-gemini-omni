Last updated on 2026-08-01 03-38-26

# Awesome Gemini Omni

[![Awesome](https://cdn.rawgit.com/sindresorhus/awesome/d7305f38d29fed78fa85652e3a63e154dd8e8829/media/badge.svg)](https://github.com/sindresorhus/awesome) [![License: CC BY 4.0](https://img.shields.io/badge/License-CC_BY_4.0-lightgrey.svg)](https://creativecommons.org/licenses/by/4.0/) [![GitHub stars](https://img.shields.io/github/stars/ZeroLu/awesome-gemini-omni?style=social)](https://github.com/ZeroLu/awesome-gemini-omni/stargazers)

| [English](./README.md) | [简体中文](./README-zh.md) |

> A curated collection of Gemini Omni prompts, bookmarked X demos, and downloadable reference videos.

This repository focuses on Gemini Omni examples surfaced from real X bookmarks. It keeps the exact prompt fragments we could recover from the posts, saves the original demo videos locally, and separates prompt-bearing posts from comparison or announcement posts.

## Table of Contents

1. [Prompt Cases](#1-prompt-cases)
2. [Comparison & Showcase Videos](#2-comparison--showcase-videos)
3. [Prompt Extraction Notes](#3-prompt-extraction-notes)
4. [Resources](#4-resources)
5. [Contributing](#5-contributing)

---

## 1. Prompt Cases

### 1.1. London Eye Refocus Demo
*A concise Gemini Omni Flash prompt that tests world knowledge, handheld motion, zoom behavior, and refocusing.*

**Prompt:**
```text
a recording from a capsule on the london eye, a jerky zoom into something in the distance and then refocusing (with a bit of back and forth) (no timestamp or dialog)
```

**Video:**

https://github.com/user-attachments/assets/a0fa2abb-c816-4d62-a44c-f5c7550886b4

Original download: [fofrai-2056503814874861569.mp4](./videos/fofrai-2056503814874861569.mp4)

*Source: fofr ([@fofrAI](https://x.com/fofrAI)) - [Post](https://x.com/fofrAI/status/2056789242274259242)*

### 1.2. Mathematical Proof Whiteboard Demo
*A Gemini Omni prompt fragment preserved from a bookmarked X post. The exported X metadata truncates the sentence, so this repository keeps the exact recoverable wording rather than inventing the missing tail.*

**Prompt fragment from X metadata:**
```text
A professor writes out a mathematical proof for
```

**Videos:**

https://github.com/user-attachments/assets/023bba16-bccb-492f-a7f3-bca668a4e5d3

Original download: [tomlikesrobots-1-2053844770892439552.mp4](./videos/tomlikesrobots-1-2053844770892439552.mp4)

https://github.com/user-attachments/assets/4a27b53c-a06d-4f81-a49b-5bb922ced286

Original download: [tomlikesrobots-2-2053844850789756928.mp4](./videos/tomlikesrobots-2-2053844850789756928.mp4)

https://github.com/user-attachments/assets/d0b0f2a1-65e2-40db-a8a4-c43da32d9e30

Original download: [tomlikesrobots-3-2053823837263839232.mp4](./videos/tomlikesrobots-3-2053823837263839232.mp4)

*Source: TomLikesRobots ([@TomLikesRobots](https://x.com/TomLikesRobots)) - [Post](https://x.com/TomLikesRobots/status/2053845600051798065)*

## 2. Comparison & Showcase Videos

### 2.1. Seedance 2.0 Fast vs Gemini Omni
*A bookmarked comparison focused on video editing power, physics, and water simulation rather than combat-style spectacle.*

**Key text:**
```text
This time Seedance 2.0 Fast vs Google Gemini Omni.

No fighting or combat this time—we are testing pure video editing power, physics, water sim and more!
```

**Preview video:**

https://github.com/user-attachments/assets/0e3786b7-7f99-48b6-8bae-56cf77457ce0

Original download: [jsfilmz-2057117155280191488.mp4](./videos/jsfilmz-2057117155280191488.mp4)

*Source: JSFILMZ ([@JSFILMZ0412](https://x.com/JSFILMZ0412)) - [Post](https://x.com/JSFILMZ0412/status/2057118348199018873)*

### 2.2. Google DeepMind Official Showcase
*The official Gemini Omni launch reel from Google DeepMind.*

**Key text:**
```text
Build your next story with Gemini Omni.
```

**Preview video:**

https://github.com/user-attachments/assets/f4e7b633-9cf0-42cd-a4de-9c374c62c22f

Original download: [googledeepmind-2056885754996133888.mp4](./videos/googledeepmind-2056885754996133888.mp4)

*Source: Google DeepMind ([@GoogleDeepMind](https://x.com/GoogleDeepMind)) - [Post](https://x.com/GoogleDeepMind/status/2056893980751917178)*

## 3. Prompt Extraction Notes

- `fofrAI` contains a clean, reusable prompt and is preserved verbatim.
- `TomLikesRobots` clearly labels the post with `Prompt:` but the exported X metadata truncates the sentence after `A professor writes out a mathematical proof for`. This repository intentionally keeps the exact recoverable fragment instead of guessing.
- `JSFILMZ` and `Google DeepMind` are useful bookmarked videos, but they are descriptive showcase posts rather than prompt posts.

## 4. Resources

- Prompt notes: [prompts/x-bookmarks.md](./prompts/x-bookmarks.md)
- Official launch post: https://x.com/GoogleDeepMind/status/2056893980751917178
- Official Google blog: https://blog.google/innovation-and-ai/models-and-research/gemini-models/gemini-omni/
- Try Gemini Omni in China on Cyberbara: https://cyberbara.com/zh/gemini-omni

## 5. Contributing

If you find more Gemini Omni prompts, better prompt reconstructions, or additional X demos worth archiving, open a PR with:

- the original source link
- the exact prompt text as posted
- downloaded media or a stable public mirror
- a short note explaining why the example is useful
